...
// @downloadURL  https://raw.githubusercontent.com/achma-learning/userscripts/main/google-custom.js
// @updateURL    https://raw.githubusercontent.com/achma-learning/userscripts/main/google-custom.js
// ==/UserScript==

/*
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║                     google-custom v4.0.0                        ║
 * ║    privacy-respecting · focus-first · digital-minimalist        ║
 * ╠══════════════════════════════════════════════════════════════════╣
 ...
 * ║  MODULE 4:  YouTube Ad Skip / Mute / SpeedUp (FairBlock-hardened)║
 ...
 *          successor to "Google Privacy & YouTube Enhancement Suite".
 *
 *  NEW     — MODULE 13: YouTube Playback Resume
 *            • Stores per-videoId currentTime in localStorage (no GM_)
 *            • Saves on timeupdate, throttled to 1.5s minimum interval
 *            • Restores on loadedmetadata; skips if URL has ?t=/start=
 *              or stored position is within last 30s of duration (95%+)
 *            • Prunes oldest entries when cache exceeds 500 videos
 *            • Toggle in settings, default ON
 *  NEW     — MODULE 14: Drive/Gmail/Search URL Unwrap
 *            • Unwraps Google's out-of-domain redirect wrappers in-place:
 *              /url?q=… (Search), drive.google.com/url?q=…,
 *              www.google.com/url?sa=t&url=…, mail.google.com /url?q=…
 *            • Rewrites href on capture-phase mousedown so middle-click,
 *              ctrl-click, and "copy link" all hit the real destination
 *            • Strips ?usp=sharing, ?usp=drive_link, &usg=… tokens from
 *              outbound Drive links to prevent Drive ID leaks
 *  IMPROVED— MODULE 4: ad mute hardened with FairBlock-style class check
 *            (#movie_player.ad-interrupting / .ad-showing) before
 *            saving pre-ad mute state — eliminates the race where the
 *            mute button click happens before YouTube applies the class.
 *            Pre-ad volume is also captured and restored.
 *  IMPROVED— Settings panel splits Privacy & Ad Handling into clearer
 *            sections; v4 toggles for Playback Resume + URL Unwrap.
 *  CHANGED — Settings storage key bumped to '__gcustom_settings_v4__'
 *            (old '__pyt_suite_settings__' values are auto-migrated on
 *            first load — no user re-config needed).
 *  CHANGED — Duplicate-run guard renamed to __GOOGLE_CUSTOM_LOADED__.
 *
...
if (window.__GOOGLE_CUSTOM_LOADED__) return;
window.__GOOGLE_CUSTOM_LOADED__ = true;
...
// ============================================================
const SETTINGS_KEY = '__gcustom_settings_v4__';
const LEGACY_SETTINGS_KEY = '__pyt_suite_settings__'; // v3 key — auto-migrated once
...
function _loadSettings() {
    // Try v4 key first; fall back to v3 key once for one-time migration.
    let raw = null;
    ...
    if (!raw) {
        try {
            ...
            if (legacy) { raw = legacy; migrated = true; }
        } catch(e) {}
    }
    try {
        ...
        if (stored && typeof stored === 'object') {
            ...
            for (var k2 in stored) if (k2 in DEFAULT_SETTINGS) merged[k2] = stored[k2];
            // Persist under the new key so v4 owns the data going forward.
            if (migrated) { try { localStorage.setItem(SETTINGS_KEY, JSON.stringify(merged)); } catch(e) {} }
            return merged;
        }
    } catch(e) {}
...
function saveAndCloseSettings() {
    ...
    if (ENV.isYouTube) {
        ...
        // Re-attach playback resume on toggle-on (toggle-off naturally no-ops via the flag check).
        if (SUITE_SETTINGS.playbackResume && typeof _resumeAttach === 'function') _resumeAttach();
    }
...
// ============================================================
// MODULE 14: DRIVE / GMAIL / SEARCH URL UNWRAP
// ============================================================
// Google wraps every outbound link in a redirector that pings their
// servers before forwarding. This module rewrites those wrappers to
// the real destination on capture-phase mousedown, BEFORE the click
// handler can fire the ping. Result:
//
//   • middle-click, Ctrl/Cmd-click, "copy link" all hit the real URL
//   • no ping → no tracking referer → faster navigation
//   • Drive share-link tokens (?usp=sharing, ?usp=drive_link, &usg=…)
//     are stripped to avoid leaking IDs in copy-paste
//
// Scoped to Google domains only (Search, Drive, Gmail, etc.). Module 1
// (Google Anti-Track) handles inline rwt() handlers; this module is
// complementary and focuses on wrapper-URL rewriting.
if (ENV.isGoogleDomain) {
_safeRun('Module14:UrlUnwrap', function() {
    if (!SUITE_SETTINGS.urlUnwrap) return;

    // Patterns that indicate a Google redirect wrapper. Returns the
    // unwrapped target URL or null if the link isn't a wrapper.
    function unwrapHref(href) {
        if (!href || typeof href !== 'string') return null;
        // Skip obvious non-wrappers and protocol-relative anchors.
        if (href.startsWith('#') || href.startsWith('javascript:') || href.startsWith('mailto:')) return null;

        let url;
        try {
            // Normalize: support both relative ("/url?q=…") and absolute forms.
            url = new URL(href, location.origin);
        } catch(e) { return null; }

        // Only handle google.* hosts (and the current origin if it's google).
        const host = url.hostname.toLowerCase();
        const isGoogleHost = /(^|\.)google\.[a-z.]+$/.test(host) || host === location.hostname;
        if (!isGoogleHost) return null;

        // /url?q=…  /url?url=…  — used by Search, Drive, Gmail, Maps
        ...
        // Drive specific: drive.google.com/uc?export=download&id=… is fine,
        // but drive.google.com/url?q=… is a redirector. Already handled above.

        return null;
    }

    // Strip Drive share-link tokens that leak file/folder IDs to anyone
    // the link is forwarded to. Only operates on drive.google.com URLs.
    function cleanDriveTokens(href) {
        if (!href) return href;
        try {
            const u = new URL(href, location.origin);
            if (!/(^|\.)drive\.google\.[a-z.]+$/.test(u.hostname.toLowerCase())) return href;
            let changed = false;
            ['usp', 'usg', 'authuser', 'pli'].forEach(function(k) {
    ...
    function rewriteAnchor(a) {
        if (!a || a.dataset.gcustomUnwrapped === '1') return;
        const original = a.getAttribute('href');
        if (!original) return;
        const unwrapped = unwrapHref(original);
        const target = unwrapped || cleanDriveTokens(original);
        if (target && target !== original) {
            a.setAttribute('href', target);
            a.dataset.gcustomUnwrapped = '1';
        }
    }

    // Capture-phase mousedown: rewrites BEFORE Google's click handlers
    // (which would fire the redirector) get a chance to run.
    function onPointerDown(e) {
        let el = e.target;
        // Walk up to find the anchor (target may be a child img/span).
        for (let depth = 0; el && depth < 5; depth++) {
            if (el.tagName === 'A' && el.href) { rewriteAnchor(el); return; }
            el = el.parentElement;
        }
    }
    document.addEventListener('mousedown', onPointerDown, true);
    document.addEventListener('auxclick', onPointerDown, true);  // middle-click
    document.addEventListener('contextmenu', onPointerDown, true); // "copy link"

    // Periodic sweep — catches programmatically-rewritten links that
    // Google injects after the page loads (Drive sidebar, Gmail thread).
    let sweepTimer = null;
    function sweep() {
        const anchors = document.querySelectorAll('a[href]:not([data-gcustom-unwrapped])');
        for (let i = 0; i < anchors.length; i++) rewriteAnchor(anchors[i]);
    }
    ...
    registerMutationCallback(scheduleSweep);
}); // END Module 14
} // end if Google domain
...
// ============================================================
// MODULE 4: Ad Skip + Mute v4 (FairBlock-hardened)
// ============================================================
// Detection: combine .ad-showing on body + `ad-interrupting` on the
// player element + classic ad UI selectors. Pre-ad mute AND volume are
// captured the very first frame the player enters ad state, eliminating
// a v3 race where mute was applied before YouTube tagged the player.
let _cachedPlayerEl = null, _lastAdState = false, _playerMutedBeforeAd = null, _preAdPlaybackRate = 1, _preAdVolume = null;

function _isAdInterruptingPlayer() {
    // FairBlock-style: the player itself flips `ad-interrupting` whenever
    // the inline ad video is being played. This survives DOM moves of
    // .ad-showing and is language/locale-independent.
    const p = document.querySelector('#movie_player') || document.querySelector('.html5-video-player');
    if (!p || !p.className) return false;
    return /\b(ad-interrupting|ad-showing)\b/.test(p.className);
}
...
function skipVideoAd() {
    ...
    // FairBlock-style class probe — covers cases where .ad-showing isn't yet present
    // but the player is already running ad video.
    var adInterrupting = (!adShowing && !pieCountdown && !surveyQuestions) ? _isAdInterruptingPlayer() : false;
    var isInAd = !!(adShowing || pieCountdown || surveyQuestions || adInterrupting);
    ...
    if (!isInAd) {
        if (_lastAdState && adVideo) {
            ...
            // Restore volume only if we actually changed it (we set volume=0 during ads).
            if (_preAdVolume !== null && adVideo.volume !== _preAdVolume) {
                try { adVideo.volume = _preAdVolume; } catch(e) {}
            }
            _playerMutedBeforeAd = null; _preAdVolume = null;
        }
        _lastAdState = false; return;
    }

    if (!_lastAdState && adVideo) {
        if (_playerMutedBeforeAd === null) {
            _playerMutedBeforeAd = _isMutedViaButton();
            if (_playerMutedBeforeAd === null) _playerMutedBeforeAd = adVideo.muted;
            _preAdPlaybackRate = adVideo.playbackRate;
            _preAdVolume = adVideo.volume;
        }
    }
...
// MODULE 13: YouTube Playback Resume (Plox-style, local-only)
// ============================================================
// Stores per-video playback position in localStorage so navigating
// away and back resumes near where you stopped. No login required, no
// network calls, no telemetry — purely a local index.
//
// Save:    on `timeupdate`, throttled to MIN_SAVE_INTERVAL ms.
// Restore: on `loadedmetadata`, exactly once per navigation, only if:
//          - URL has no t=, start=, or time_continue= override
//          - stored position > MIN_RESUME seconds (don't restore from 0)
//          - stored position < (duration - END_GUARD) (don't restore from end)
// Cap:     RESUME_MAX_VIDEOS oldest entries pruned when exceeded.
const RESUME_KEY        = '__gcustom_resume__';
const RESUME_MIN_SAVE   = 1500;  // ms between writes
const RESUME_MIN_TIME   = 5;     // s — below this we don't bother saving/restoring
const RESUME_END_GUARD  = 30;    // s before duration end — don't restore (counts as "watched")
const RESUME_MAX_VIDEOS = 500;

let _resumeData = null;
let _resumeVideo = null;
let _resumeVideoId = null;
let _resumeRestoredFor = null;       // videoId we already restored on this navigation
let _resumeLastSaveTs = 0;
let _resumeAttachedTo = null;        // current video element with our listeners
...
function _resumeLoad() {
    ...
    try {
        ...
        if (parsed && parsed.videos && Array.isArray(parsed.order)) {
            _resumeData = parsed;
            return _resumeData;
        }
    } catch(e) {}
    _resumeData = { videos: {}, order: [] };
    return _resumeData;
}

function _resumePersist() {
    try { localStorage.setItem(RESUME_KEY, JSON.stringify(_resumeData)); } catch(e) {}
}

function _resumeGetVideoId() {
...
function _resumeHasUrlTimeOverride() {
    try {
        const p = new URLSearchParams(location.search);
        // Any of these means user explicitly chose a time — respect it.
        return !!(p.get('t') || p.get('start') || p.get('time_continue'));
    } catch(e) { return false; }
}

function _resumeRecord(videoId, t, duration) {
    const data = _resumeLoad();
    // Treat near-end (>=95%) as "completed" — drop instead of save so we
    // don't resume at the very end on next visit.
    const completed = duration && (t >= duration - RESUME_END_GUARD);
    if (completed) {
        if (data.videos[videoId]) {
            delete data.videos[videoId];
            const idx = data.order.indexOf(videoId);
            if (idx !== -1) data.order.splice(idx, 1);
            _resumePersist();
        }
        return;
    }
    if (t < RESUME_MIN_TIME) return; // not enough watched to bother
    data.videos[videoId] = { t: Math.floor(t), d: Math.floor(duration || 0), u: Date.now() };
    const idx = data.order.indexOf(videoId);
    if (idx !== -1) data.order.splice(idx, 1);
    data.order.push(videoId);
    while (data.order.length > RESUME_MAX_VIDEOS) {
        const evict = data.order.shift();
        if (evict) delete data.videos[evict];
    }
    _resumePersist();
}

function _resumeOnTimeUpdate() {
    if (!SUITE_SETTINGS.playbackResume) return;
    if (!_resumeVideo || !_resumeVideoId) return;
    const now = Date.now();
    if (now - _resumeLastSaveTs < RESUME_MIN_SAVE) return;
    _resumeLastSaveTs = now;
    if (document.querySelector('.ad-showing') || _isAdInterruptingPlayer()) return;
    if (!_resumeVideo.duration || isNaN(_resumeVideo.duration)) return;
    _resumeRecord(_resumeVideoId, _resumeVideo.currentTime, _resumeVideo.duration);
}

function _resumeOnLoadedMetadata() {
    if (!SUITE_SETTINGS.playbackResume) return;
    if (!_resumeVideo || !_resumeVideoId) return;
    if (_resumeRestoredFor === _resumeVideoId) return;
    _resumeRestoredFor = _resumeVideoId;
    if (_resumeHasUrlTimeOverride()) return;
    const data = _resumeLoad();
    const entry = data.videos[_resumeVideoId];
    if (!entry) return;
    const dur = _resumeVideo.duration;
    if (!dur || isNaN(dur)) return;
    if (entry.t < RESUME_MIN_TIME) return;
    if (entry.t > dur - RESUME_END_GUARD) return;
    try { _resumeVideo.currentTime = entry.t; } catch(e) {}
}

function _resumeAttach() {
    if (!SUITE_SETTINGS.playbackResume) return;
    if (location.pathname !== '/watch') return;
    const videoId = _resumeGetVideoId();
    if (!videoId) return;
    const vid = document.querySelector('video.html5-main-video');
    if (!vid) return;
    // If we're already attached to this exact video for this id, nothing to do.
    if (_resumeAttachedTo === vid && _resumeVideoId === videoId) return;
    // Detach previous (handles SPA video element replacement).
    if (_resumeAttachedTo && _resumeAttachedTo !== vid) {
        _resumeAttachedTo.removeEventListener('timeupdate', _resumeOnTimeUpdate);
        _resumeAttachedTo.removeEventListener('loadedmetadata', _resumeOnLoadedMetadata);
    }
    _resumeVideo = vid;
    _resumeVideoId = videoId;
    _resumeAttachedTo = vid;
    _resumeLastSaveTs = 0;
    vid.addEventListener('timeupdate', _resumeOnTimeUpdate);
    vid.addEventListener('loadedmetadata', _resumeOnLoadedMetadata);
    // If metadata is already loaded (SPA back-nav), try restore now.
    if (vid.readyState >= 1) _resumeOnLoadedMetadata();
}

// Save once on unload/visibility hide as a safety net (timeupdate may have
// just been throttled out in the last second).
function _resumeFlush() {
    if (!SUITE_SETTINGS.playbackResume) return;
    if (!_resumeVideo || !_resumeVideoId) return;
    if (!_resumeVideo.duration || isNaN(_resumeVideo.duration)) return;
    _resumeRecord(_resumeVideoId, _resumeVideo.currentTime, _resumeVideo.duration);
...
document.addEventListener('visibilitychange', function() {
    if (document.visibilityState === 'hidden') _resumeFlush();
});
...
function startAllYTSystems() {
    ...
    function onNavigate() {
        ...
        // Reset resume state for the new navigation; _resumeAttach below will
        // re-attach to the new video element / videoId.
        _resumeRestoredFor = null;
        requestAnimationFrame(function() {
            ...
            setTimeout(_resumeAttach, 500);
            setTimeout(_resumeAttach, 1500);
        });
    }
    ...
    setInterval(function() {
        if (location.pathname === '/watch') { fixWatchPageScroll(); observeScrollElements(); applyDefaultSpeed(); _resumeAttach(); }
    }, 5000);

    document.addEventListener('keydown', handleYTKeys);

    setTimeout(function() { applyDefaultSpeed(); applyAutoTheater(); applyAutoFullscreen(); initRemainingTime(); _resumeAttach(); }, 2000);
}
